...
def _addr_keys(df: pl.DataFrame, idc: str, n_nums: int, n_toks: int, slice_rows: int = 1_000_000) -> pl.DataFrame:
    """Keys are built in row slices: the exploded (number x word) strings of ~10M records do not
    fit in 15 GB at once. Output is identical to a single pass."""
    if df.height > slice_rows:
        return pl.concat([_addr_keys(df.slice(lo, slice_rows), idc, n_nums, n_toks, slice_rows)
                          for lo in range(0, df.height, slice_rows)]).unique()
    toks = (
