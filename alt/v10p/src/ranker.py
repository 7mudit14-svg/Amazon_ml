...
def build_index(cfg: Config, split: str, log=print) -> dict:
    ...
    import countscale
    if countscale.enabled():
        f = countscale.factors(s1, tg)
        ix["s_tscale"] = s1.join(f, on="country", how="left").sort("s1")["t_scale"].to_numpy()
...
def union_chunk(ix: dict, lo: int, hi: int) -> pl.DataFrame:
    ...
    if "s_tscale" in ix:   # countscale: union and block sizes at train pool scale
        sc = pl.Series("_sc", ix["s_tscale"][u["s1"].to_numpy()], dtype=pl.Float32)
        u = u.with_columns(sc).with_columns(
            [(pl.col(c) * pl.col("_sc")).alias(c) for c in ("e_bs", "addr_bs", "cc_bs")])
    else:
        u = u.with_columns(_sc=pl.lit(1.0, pl.Float32))
    u = u.with_columns(
        ...
        n_union=(pl.len().over("s1").cast(pl.Float32) * pl.col("_sc")).log1p(),
