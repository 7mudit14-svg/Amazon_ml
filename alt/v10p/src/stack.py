...
def _scale_df(df: pl.DataFrame, tg: pl.DataFrame) -> pl.DataFrame:
    import countscale
    if not countscale.enabled():
        return df
    f = countscale.factors(tg.select("country"), tg.select("country"))
    return df.join(f, on="country", how="left").with_columns(df=pl.col("df") * pl.col("t_scale")).drop("s_scale", "t_scale")
